...
class conway:
    ...
    def evolve(self, rule):
        next_state = conway(self.r, self.c, 'zeros')
        for r in range(0, self.r, 1):
            for c in range(0, self.c, 1):
                value = self.storage[r][c]
                valout = rule(value, self.getNeighbours(r,c))
                next_state.setPos(r,c,valout)
        self.storage = next_state.storage
        return True
